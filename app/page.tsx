...
function ModelComparison() {
  const models = [
    { name: 'Prompt only', detail: 'Qwen3-4B · zero-shot', result: 'Not run', scope: 'Planned baseline', status: 'Pending' },
    { name: 'RAG', detail: 'Local deterministic + hybrid index', result: '4/4 gates', scope: '6-case smoke test', status: 'Selected' },
    { name: 'QLoRA + RAG', detail: '4-bit adapter + race context', result: 'No result', scope: '0 / 20 approved examples', status: 'Gated' },
  ];
  return <div><div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-primary">Honest model status</p><h3 className="mt-2 text-2xl font-semibold">Which approach currently earns the publish button?</h3></div><Link href="/observability" className="hidden text-xs text-primary hover:underline sm:block">Inspect the full evidence →</Link></div><div className="mt-8 overflow-x-auto"><div className="min-w-[680px]"><div className="grid grid-cols-[1.5fr_repeat(3,1fr)] border-b border-white/10 px-4 pb-3 text-[9px] font-bold uppercase tracking-[.14em] text-white/30"><span>Variant</span><span>Measured result</span><span>Evidence scope</span><span>Decision</span></div>{models.map((model) => <div key={model.name} className={`grid grid-cols-[1.5fr_repeat(3,1fr)] items-center border-b border-white/6 px-4 py-5 text-xs ${model.status === 'Selected' ? 'bg-primary/[.06]' : ''}`}><div><p className="font-semibold">{model.name}</p><p className="mt-1 text-[10px] text-white/30">{model.detail}</p></div><span className="font-mono text-primary">{model.result}</span><span className="text-[10px] text-white/50">{model.scope}</span><Badge variant="outline" className={model.status === 'Selected' ? 'w-fit border-primary/30 text-primary' : model.status === 'Gated' ? 'w-fit border-amber-300/20 text-amber-200' : 'w-fit border-white/10 text-white/40'}>{model.status}</Badge></div>)}</div></div><p className="mt-6 text-xs leading-6 text-white/35">The QLoRA code path is implemented, but training is intentionally blocked until at least 20 human-approved drafts exist. No adapter performance is claimed.</p></div>;
}
...
function AgentTrace() {
  const trace = [
    ...
    ['05', 'safety_editor', 'Blocked unsupported health and intent inferences', '1 ms'],
